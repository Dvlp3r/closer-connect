...
const float SliderMinValue = 0.0;
const float SliderMaxValue = 1.0;
const float SliderMinimumRangeLength = 0.01;
...
- (id)initWithFrame:(CGRect)frame {
    if ((self = [super initWithFrame:CGRectMake(frame.origin.x, frame.origin.y, frame.size.width, SLIDER_HEIGHT)])) {
        ...
        trackImageView = [[UIImageView alloc] initWithFrame:CGRectMake(0, 0, frame.size.width, SLIDER_HEIGHT)];
...
- (void)setDefaults {
    min = SliderMinValue;
    max = SliderMaxValue;
    minimumRangeLength = SliderMinimumRangeLength;
    
    [self setMinThumbImage:[UIImage imageNamed:@"slider"]];
    [self setMaxThumbImage:[UIImage imageNamed:@"slider"]];
    
    [self setTrackImage:[[UIImage imageNamed:@"fullrange.png"] resizableImageWithCapInsets:UIEdgeInsetsMake(0, 0, 0, 0)]];
    [self setInRangeTrackImage:[UIImage imageNamed:@"fillrange.png"]];
}
...
- (void)setInRangeTrackImage:(UIImage *)image {
   	trackImageView.frame = CGRectMake(inRangeTrackImageView.frame.origin.x - 12, trackImageView.frame.origin.y, inRangeTrackImageView.frame.size.width + 24, trackImageView.frame.size.height);
...
- (void)setTrackImage:(UIImage *)image {
    trackImageView.frame = CGRectMake(0, MAX((self.frame.size.height-image.size.height)/2.0,0), self.frame.size.width, MIN(self.frame.size.height,image.size.height));
