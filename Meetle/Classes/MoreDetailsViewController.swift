        ...
        else
        {
            ...
            DispatchQueue.global(qos: .background).async {
                if let user = Auth.auth().currentUser
                {
                    ...
                    {
                        self.ref.child("users").child(user.uid).setValue(["Name": self.nameTextField.text!, "DOB": self.dobTextField.text!,  "Gender": self.genderSegment.selectedSegmentIndex,  "Latitude": self.currentLocation.coordinate.latitude,  "Longitude": self.currentLocation.coordinate.longitude])
                        DispatchQueue.main.async {
                            self.stopAnimating()
